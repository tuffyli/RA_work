...
# 0. Helpers ----
# ---------------------------------------------------------------------------- #
# Defines reusable formatting and data-construction helpers used later in the
# regression tables and sample-comparison sections.
...
add_inpe_prova_1819 <- function(base, inpe_path = inpe_mun_path) {
...
# ---------------------------------------------------------------------------- #
# 1. Start ----
# Main regression workflow: loads prepared data, builds analysis panels, runs RD specifications, and writes tables/figures.
# ---------------------------------------------------------------------------- #

# ---------------------------------------------------------------------------- #
## 1.0 Data -----
# Loads the final student-level database produced by the data pipeline.
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
### 1.1.1 Aux Base ----
# Keeps auxiliary differenced covariates used later for robustness checks and table construction.
# ---------------------------------------------------------------------------- #
...
list[[as.character(paste0(2019,"-",2018,"|fuso+temp+house"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_res$dn_gel[base_res$ano == 2019], # refrigerator
...
# Stores the main residence-based RD bandwidth for later graphs and robustness
# sections so every downstream block uses the same cutoff.
bw_main_r  <- list[["2019-2018|all"]]$bws[1]
bw_bias_r  <- list[["2019-2018|all"]]$bws[2]

# Save the main bandwidth
save(bw_main_r, bw_bias_r,
     file = file.path(bandwidth_path, "data_bandwidth_2019_2018_residence_all.RData"))
# Original output: bandwidths_2019_2018_Res_all.RData
# ---------------------------------------------------------------------------- #
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_main_residence_controls_v01.tex"))
# Original output: DIFF_Principal_TC_v5.tex

rm(latex_table, t10, result, list)
# ---------------------------------------------------------------------------- #
## 1.2 Dists -----
# Repeats the core setup using alternative distance definitions and placebo year comparisons.
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
### 1.2.1 2018-2017 ----
# Builds the pre-treatment 2018-2017 comparison used as a placebo/reference exercise.
#### 1.2.1.1 Data ----
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
#### 1.2.2.2 Regression ----
# Runs RD specifications for the 2018-2017 distance comparison.
# ---------------------------------------------------------------------------- #
...
list[[as.character(paste0(2018,"-",2017,"prova|0"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_a$dn_gel[base_a$ano == 2018], # refrigerator
    base_a$dn_cel[base_a$ano == 2018], # cellphone
...
list[[as.character(paste0(2018,"-",2017,"esc|0"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_esc$dn_gel[base_esc$ano == 2018], # refrigerator
    base_esc$dn_cel[base_esc$ano == 2018], # cellphone
...
# Recreates the main table structure for exam- and school-municipality variants.
# ---------------------------------------------------------------------------- #
#### 1.2.4.1 Data ----
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
#### 1.2.4.2 EXAM ----
# Uses municipality of exam as the running-variable geography.
# ---------------------------------------------------------------------------- #
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_main_exam_v01.tex"))
# Original output: prova_principal.tex
rm(ef, result, t10, latex_table)

# ---------------------------------------------------------------------------- #
#### 1.2.4.3 School ----
# Uses municipality of school as the running-variable geography.
# ---------------------------------------------------------------------------- #
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_main_school_v01.tex"))
# Original output: esc_principal.tex
...
# ---------------------------------------------------------------------------- #
## 1.3 Main 2018-2017 ----
# Prepares the 2018-2017 main placebo panel and aligns it with the main specification format.
# ---------------------------------------------------------------------------- #
...
list[[as.character(paste0(2018,"-",2017,"|fuso+temp+house"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_res$dn_gel[base_res$ano == 2018], # refrigerator
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2018_2017_main_placebo_v01.tex"))
...
# Creates external datasets and visual diagnostics from the regression-ready panels.
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
## 2.1 Stata Export ----
# Builds a compact municipality-year dataset for use outside R, especially in Stata/CMOGRAM workflows.
# ---------------------------------------------------------------------------- #
...
base <- base %>%
  ...
  mutate(
    ...
    time13 = case_when(
      # States that follow UTC-3 (Brasilia time)
...
df_time1 <- base %>%
  ...
  mutate(
    ...
    v1_nota = ifelse(ano == 2018, media, NA),
    v2_nota = max(v1_nota, na.rm = TRUE),
    dmedia_bra1 = media - v2_nota   # difference for UTC-3 (Brasilia)
  ) %>%
  ungroup() %>%
  filter(dup2 == 2) %>%
  select(-c(dup2, dup1, v1_nota, v2_nota))
...
# Writes the prepared municipality-level export file.
# ---------------------------------------------------------------------------- #
library(haven)

write_dta(df_cmo, file.path(stata_output_path, "data_cmogram_municipality_year.dta"))
# Original output: dados_cmogram.dta

# ---------------------------------------------------------------------------- #
## 2.2 Within Bandwidth Graph ----
# Produces polynomial fit figures restricted to the selected RD bandwidths.
# ---------------------------------------------------------------------------- #
### 2.2.1 1918 ----
# ---------------------------------------------------------------------------- #
...
for (i in fig_loop) {
  ...
  ggsave(filename = path_paste(controls_figures_path, i, "_rdplot_2019_2018_pol2_v01.png"),plot = fig_gg, device = "png",dpi = 300, height = 9, width = 13)
# Original output: {i}_pol2_1918_v1.png
  ggsave(filename = path_paste(controls_pdf_path, i, "_rdplot_2019_2018_pol2_v01.pdf"),plot = fig_gg, device = "pdf",height = 9, width = 13)
# Original output: {i}_pol2_1918_v1.pdf
...
list[[as.character(paste0(2018,"-",2017,"C|NF"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_c$dn_gel[base_c$ano == 2018], # refrigerator
...
for (i in fig_loop) {
  ...
  ggsave(filename = path_paste(controls_figures_path, i, "_rdplot_2018_2017_pol2_v01.png"),plot = fig_gg, device = "png",dpi = 300, height = 9, width = 13)
# Original output: {i}_pol2_1817_v1.png
  ggsave(filename = path_paste(controls_pdf_path, i, "_rdplot_2018_2017_pol2_v01.pdf"),plot = fig_gg, device = "pdf",height = 9, width = 13)
# Original output: {i}_pol2_1817_v1.pdf
...
# ---------------------------------------------------------------------------- #
# 3. Subjects ----
# Estimates treatment effects separately by ENEM subject area.
# ---------------------------------------------------------------------------- #
# Bases
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_subjects_v01.tex"))
# Original output: Materias_v1.tex
#5548
rm(result, latex_table, notas_tab, d_list, p_list)

# ---------------------------------------------------------------------------- #
# 4. Essay ----
# Estimates treatment effects for essay score and essay component outcomes.
# ---------------------------------------------------------------------------- #

# Aggregates by criteria
base_res <- base %>% 
  filter(priv0 == 1,
         ano %in% c(2018:2019)) %>%
  group_by(mun_res,ano,dist_hv_res,seg_res,lat_res,lon_res) %>%
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_essay_v01.tex"))
# Original output: Redacao_v1.tex
#5548
...
# ---------------------------------------------------------------------------- #
# 5. Difficulty ----
# Studies item difficulty measures, including easy-hard contrasts and mother-education splits.
# ---------------------------------------------------------------------------- #
gc()
# Aggregates by criteria
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_item_difficulty_v01.tex"))
# Original output: Dificuldade_v1.tex
    ...
    result[[as.character(paste0(i,"_",v))]] <-
      rdrobust(
        ...
        covs = cbind(
          ...
          temp_ag$dn_gel[temp_ag$ano == 2019], # refrigerator
          temp_ag$dn_cel[temp_ag$ano == 2019], # cellphone
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_item_difficulty_mother_education_v01.tex"))
# Original output: Dificuldade_mae.tex
#0 = 5465, 1 = 5433
rm( mat_nota, result, latex_table)

# ---------------------------------------------------------------------------- #
# 6. Exam Days (comparability) ----
# Compares outcomes across exam days to check whether effects differ by test-day composition.
# ---------------------------------------------------------------------------- #
## 6.1 Data ----
# ---------------------------------------------------------------------------- #
# 2019 - 2018 Data
...
base_res2 <- base_res2 %>%
  ...
  filter(dup2 == 2) %>% 
  select(-c(v1_d1, v2_d1, v2_d2, v1_d2, dup1, dup2)) %>% 
  left_join(base_aux2, by = c("mun_res", "ano"))
...
for (i in d_list){
  ...
  result[[as.character(paste0(i,"|19-18"))]] <-
    rdrobust(
      y = base_res[[i]][base_res$ano == 2019],
      x = base_res$dist_hv_res[base_res$ano == 2018],
      c = 0,
...
for (i in d_list){
  ...
  ef <- dummy_cols(base_res2$seg_res[base_res2$ano == 2017])
  ef <- ef %>% select(-1,-2)
  
  
  result[[as.character(paste0(i,"|18-17"))]] <-
    rdrobust(
      y = base_res2[[i]][base_res2$ano == 2018],
      x = base_res2$dist_hv_res[base_res2$ano == 2017],
      c = 0,
      h = bw_main_r,
      b = bw_bias_r,
      p = 1,
      cluster = base_res2$seg_res[base_res2$ano == 2017],
      weights = base_res2$obs[base_res2$ano == 2017],
      vce = "hc0",
      covs = cbind(
        ef, 
        base_res2$lat_res[base_res2$ano == 2017],
        base_res2$lon_res[base_res2$ano == 2017],
        
        #Weather
        base_res2$dtempd1[base_res2$ano == 2018], #Temperature
        base_res2$dumidd1[base_res2$ano == 2018], #Humidity d1
        base_res2$dumidd2[base_res2$ano == 2018], #Humidty d2
        base_res2$dtempd2[base_res2$ano == 2018], #temp2
        
        #House
        base_res2$dn_ban[base_res2$ano == 2018], #bathrooms
        base_res2$dpessoa[base_res2$ano == 2018], #people in household
        base_res2$dn_qua[base_res2$ano == 2018], #houses
        base_res2$dn_car[base_res2$ano == 2018], #cars
        base_res2$dn_gel[base_res2$ano == 2018], # refrigerator
        base_res2$dn_cel[base_res2$ano == 2018], # cellphone
        base_res2$dpc[base_res2$ano == 2018],    #pc
        base_res2$dinternet[base_res2$ano == 2018], #internet
        base_res2$dempr_dom[base_res2$ano == 2018], #Housekeeping
        
        #remaining
        base_res2$descm[base_res2$ano == 2018], #mother educ
        base_res2$dfem[base_res2$ano == 2018], #Female
        base_res2$dppi[base_res2$ano == 2018], #PPI
        base_res2$didade[base_res2$ano == 2018], #Age
        base_res2$descp[base_res2$ano == 2018], #father educ
        ...
        base_res2$dpibpc[base_res2$ano == 2018], #pibpc
...
result <- data.frame(
  ...
  `(1)` = c(#2019-2018
    ...
    # 2018-2017
    fmt_est(dias$coef[3], dias$pv[3]),
    fmt_se(dias$se[3]),
    fmt_npair(dias$n_left[3], dias$n_rght[3]),
    fmt_bw(dias$bw[3]),
    formatC(dias$totr[3] + dias$totl[3], format = "d", big.mark = ",")
  ),
  `(2)` = c(#Low
    ...
    # 2018-2017
    fmt_est(dias$coef[4], dias$pv[4]),
    fmt_se(dias$se[4]),
    fmt_npair(dias$n_left[4], dias$n_rght[4]),
    fmt_bw(dias$bw[4]),
    formatC(dias$totr[4] + dias$totl[4], format = "d", big.mark = ",")
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_exam_days_v01.tex"))
# Original output: Dias_v1.tex

rm(dias, result, d_list, latex_table, base_res2, base_aux2, base_res)


# ---------------------------------------------------------------------------- #
# 7. Absences ----
# Builds and estimates absence outcomes, including links to INPE weather controls and census counts.
# ---------------------------------------------------------------------------- #
gc()
...
base_abs <- readRDS(file = path_paste(no_age_path, "base_abs_2019.RDS")) %>%
  bind_rows(readRDS(file = path_paste(no_age_path, "base_abs_2018.RDS"))) %>%
...
#### 7.1.1.1 Data ----
# ---------------------------------------------------------------------------- #
educ <- readRDS(file.path(processed_path, "census_students.RDS"))
  ...
  result[[as.character(paste0("nc_",i,"|TC"))]] <-
    rdrobust(
      ...
      covs = cbind(
        ...
        temp$dn_qua, #houses
        temp$dn_car, #cars
        temp$dn_gel, # refrigerator
        temp$dn_cel, # cellphone
        temp$dpc,    #pc
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_absence_days_v01.tex"))
# Original output: Abs_Dias_v1.tex
#N = 5559
...
# ---------------------------------------------------------------------------- #
# 8. Heterogeneity ---- 
# Re-runs the main specifications across demographic and household subgroups.
# ---------------------------------------------------------------------------- #
...
for (v in vars_diff) {
  ...
  base_ab[[v1]] <- ifelse(base_ab$ano == 2018, base_ab[[v]], NA_real_)
  ...
  base_ab[[dv]] <- base_ab[[v]] - base_ab[[v2]]
  
  base_ab[[dv]][!is.finite(base_ab[[dv]])] <- NA
}

temp_cols <- grep("^(v1_|v2_)", names(base_ab), value = TRUE)
base_ab <- base_ab %>% select(-all_of(temp_cols)) %>% 
  mutate(across(everything(), ~ replace(.x, is.infinite(.x), NA))) %>%  #Turning INF to NA
  rename(d.media = dmedia)
...
for (v in vars_diff) {
  ...
  base_ppi[[v1]] <- ifelse(base_ppi$ano == 2018, base_ppi[[v]], NA_real_)
  ...
  base_ppi[[dv]] <- base_ppi[[v]] - base_ppi[[v2]]
  
  base_ppi[[dv]][!is.finite(base_ppi[[dv]])] <- NA
}

temp_cols <- grep("^(v1_|v2_)", names(base_ppi), value = TRUE)
base_ppi <- base_ppi %>% select(-all_of(temp_cols)) %>% 
  mutate(across(everything(), ~ replace(.x, is.infinite(.x), NA))) %>%  #Turning INF to NA
  rename(d.media = dmedia)
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_race_v01.tex"))
# Original output: Raca_v1.tex
#5417, 5336
...
for (v in vars_diff) {
  ...
  base_fem[[v1]] <- ifelse(base_fem$ano == 2018, base_fem[[v]], NA_real_)
  ...
  base_fem[[dv]] <- base_fem[[v]] - base_fem[[v2]]
  
  base_fem[[dv]][!is.finite(base_fem[[dv]])] <- NA
}

temp_cols <- grep("^(v1_|v2_)", names(base_fem), value = TRUE)
base_fem <- base_fem %>% select(-all_of(temp_cols)) %>% 
  mutate(across(everything(), ~ replace(.x, is.infinite(.x), NA))) %>%  #Turning INF to NA
  rename(d.media = dmedia)
...
for (v in vars_diff) {
  ...
  base_masc[[v1]] <- ifelse(base_masc$ano == 2018, base_masc[[v]], NA_real_)
  ...
  base_masc[[dv]] <- base_masc[[v]] - base_masc[[v2]]
  
  base_masc[[dv]][!is.finite(base_masc[[dv]])] <- NA
}

temp_cols <- grep("^(v1_|v2_)", names(base_masc), value = TRUE)
base_masc <- base_masc %>% select(-all_of(temp_cols)) %>% 
  mutate(across(everything(), ~ replace(.x, is.infinite(.x), NA))) %>%  #Turning INF to NA
  rename(d.media = dmedia)
...
for (v in vars_diff) {
  ...
  base_high[[v1]] <- ifelse(base_high$ano == 2018, base_high[[v]], NA_real_)
  ...
  base_high[[dv]] <- base_high[[v]] - base_high[[v2]]
  
  base_high[[dv]][!is.finite(base_high[[dv]])] <- NA
}

temp_cols <- grep("^(v1_|v2_)", names(base_high), value = TRUE)
base_high <- base_high %>% select(-all_of(temp_cols)) %>% 
  mutate(across(everything(), ~ replace(.x, is.infinite(.x), NA))) %>%  #Turning INF to NA
  rename(d.media = dmedia)
...
for (v in vars_diff) {
  ...
  base_low[[v1]] <- ifelse(base_low$ano == 2018, base_low[[v]], NA_real_)
  ...
  base_low[[dv]] <- base_low[[v]] - base_low[[v2]]
  
  base_low[[dv]][!is.finite(base_low[[dv]])] <- NA
}

temp_cols <- grep("^(v1_|v2_)", names(base_low), value = TRUE)
base_low <- base_low %>% select(-all_of(temp_cols)) %>% 
  mutate(across(everything(), ~ replace(.x, is.infinite(.x), NA))) %>%  #Turning INF to NA
  rename(d.media = dmedia)
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_mother_education_v01.tex"))
# Original output: Mae_Education_v1.tex
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_older_vs_young_v01.tex"))
# Original output: OLDER_vs_young_v1.tex

rm(base_a, names, result, rlist, tab, latex_table, base_mun)

# ---------------------------------------------------------------------------- #
# 9.Migration ------
# Tests whether results differ across migration/residence-school-exam municipality patterns.
# ---------------------------------------------------------------------------- #
...
var_list <- c(
  ...
  "nonmig1", # Exam municipality = residence = school
  #"nonmig2", # Exam municipality = residence != school
  "nonmig3" #, # Exam municipality != residence = school
  #"nonmig4"  # Exam municipality = school != residence
)
...
# ---------------------------------------------------------------------------- #
# 10. Time Zone ----
# Separates and tests effects by timezone-related municipality groups.
# ---------------------------------------------------------------------------- #
## 10.1 Data -----
# ---------------------------------------------------------------------------- #
...
# Municipality filter by time zone
...
for (i in time_list){
  ...
  result_time[[as.character(i)]] <- rdrobust(
    ...
    covs = cbind(
      ...
      b_temp$dn_gel[b_temp$ano == 2019], # refrigerator
      b_temp$dn_cel[b_temp$ano == 2019], # cellphone
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_timezone_v01.tex"))
# Original output: fuso_resultado_v1.tex

rm(result, result_tab, latex_table,i, result_time, time_list)

# ---------------------------------------------------------------------------- #
# 11. Segments ----
# Runs border-segment-specific checks to assess spatial heterogeneity along the HV frontier.
# ---------------------------------------------------------------------------- #
## 11.1 Data ----
# ---------------------------------------------------------------------------- #

base_y <- base[priv0 == 1,.(media = mean(media, na.rm = T),
                            obs = .N),
               by = .(mun_res,ano,dist_hv_res,seg_res,lat_res,lon_res)] %>% 
  filter(as.numeric(ano) %in% c(2018,2019)) %>% 
  arrange(mun_res,ano) %>%
...
for (s in c(1:7)){
  ...
  result_list[[as.character(paste0("not-",s,"|C"))]] <- rdrobust(
    y = base_a$d.media[base_a$ano == 2019],
    x = base_a$dist_hv_res[base_a$ano == 2018],
    c = 0,
    p = 1,
...
result_tab <- data.frame(
  coef   = sapply(result_list, function(x) x$coef[3]),
  se     = sapply(result_list, function(x) x$se[3]),
  pv     = sapply(result_list, function(x) x$pv[3]),
  n_left = sapply(result_list, function(x) x$N_h[1]),
  n_rght = sapply(result_list, function(x) x$N_h[2]),
  bw     = sapply(result_list, function(x) x$bws[1, 1]),
  totr   = sapply(result_list, function(x) x$N[2]),
  totl   = sapply(result_list, function(x) x$N[1])
)
...
result <- data.frame(
  ...
  `(1)` = c(
    ...
    fmt_est(result_tab$coef[1], result_tab$pv[1]),
    fmt_se(result_tab$se[1]),
    paste0("N$_L$ = ", fmt_n(result_tab$n_left[1]),", N$_R$ = ", fmt_n(result_tab$n_rght[1])),
    paste0("BW = ", fmt_bw(result_tab$bw[1])),
    #seg2
    fmt_est(result_tab$coef[2], result_tab$pv[2]),
    fmt_se(result_tab$se[2]),
    paste0("N$_L$ = ", fmt_n(result_tab$n_left[2]),", N$_R$ = ", fmt_n(result_tab$n_rght[2])),
    paste0("BW = ", fmt_bw(result_tab$bw[2])),
    #sef3
    fmt_est(result_tab$coef[3], result_tab$pv[3]),
    fmt_se(result_tab$se[3]),
    paste0("N$_L$ = ", fmt_n(result_tab$n_left[3]),", N$_R$ = ", fmt_n(result_tab$n_rght[3])),
    paste0("BW = ", fmt_bw(result_tab$bw[3])),
    #seg4
    fmt_est(result_tab$coef[4], result_tab$pv[4]),
    fmt_se(result_tab$se[4]),
    paste0("N$_L$ = ", fmt_n(result_tab$n_left[4]),", N$_R$ = ", fmt_n(result_tab$n_rght[4])),
    paste0("BW = ", fmt_bw(result_tab$bw[4])),
    #seg5
    fmt_est(result_tab$coef[5], result_tab$pv[5]),
    fmt_se(result_tab$se[5]),
    paste0("N$_L$ = ", fmt_n(result_tab$n_left[5]),", N$_R$ = ", fmt_n(result_tab$n_rght[5])),
    paste0("BW = ", fmt_bw(result_tab$bw[5])),
    #seg6
    fmt_est(result_tab$coef[6], result_tab$pv[6]),
    fmt_se(result_tab$se[6]),
    paste0("N$_L$ = ", fmt_n(result_tab$n_left[6]),", N$_R$ = ", fmt_n(result_tab$n_rght[6])),
    paste0("BW = ", fmt_bw(result_tab$bw[6])),
    #seg7
    fmt_est(result_tab$coef[7], result_tab$pv[7]),
    fmt_se(result_tab$se[7]),
    paste0("N$_L$ = ", fmt_n(result_tab$n_left[7]),", N$_R$ = ", fmt_n(result_tab$n_rght[7])),
    paste0("BW = ", fmt_bw(result_tab$bw[7]))
  ),
  check.names = FALSE,
  stringsAsFactors = FALSE
)
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_border_segments_v01.tex"))
# Original output: segments.tex



rm(latex_table, result_list, result_tab, base_y, result)
# ---------------------------------------------------------------------------- #
# 12. Bandwidth test ----
# Explores sensitivity of estimates to alternative bandwidth choices.
# ---------------------------------------------------------------------------- #
...
ggsave(filename = file.path(controls_figures_path, "fig_bandwidth_sensitivity_v02.png"), plot = graph,device = "png", width = 15, height = 10)
# Original output: bandwith_v2.png
ggsave(filename = file.path(controls_pdf_path, "fig_bandwidth_sensitivity_v02.pdf"), plot = graph,device = "pdf", width = 8, height = 6)
# Original output: bandwith_v2.pdf

rm(graph, tablist_bw, rlist_bw, h, b, c, base_a)

# ---------------------------------------------------------------------------- #
# 13. Balance Figs ----
# Creates covariate balance figures in levels and first differences.
# ---------------------------------------------------------------------------- #
## 13.1 Lists  -----
# ---------------------------------------------------------------------------- #
...
ggsave(plot = plot_covs, filename = file.path(scores_figures_path, "fig_balance_levels_v01.png"), device = "png", height = 13, width = 7)
# Original output: covs_test_dif_lvl.png
ggsave(plot = plot_covs, filename = file.path(scores_pdf_path, "fig_balance_levels_v01.eps"), device = "eps", height = 13, width = 7)
# Original output: covs_test_dif_lvl.eps
...
ggsave(plot = plot_covs, filename = file.path(scores_figures_path, "fig_balance_differences_v01.png"), device = "png", height = 13, width = 7)
# Original output: covs_test_dif.png
ggsave(plot = plot_covs, filename = file.path(scores_pdf_path, "fig_balance_differences_v01.eps"), device = "eps", height = 13, width = 7)
# Original output: covs_test_dif.eps

rm(covs_np, ylist, plot_covs, var_map, var_list, base_inpe_t)

# ---------------------------------------------------------------------------- #
# 14. SAEB ----
# Repeats analogous RD exercises using SAEB outcomes as an external education dataset.
# ---------------------------------------------------------------------------- #

saeb_base <- readRDS(file.path(processed_path, "saeb_total.RDS")) %>% 
  setDT()
...
writeLines(latex_table, file.path(controls_output_path, "tbl_saeb_rd_v01.tex"))
# Original output: Saeb.tex
...
# ---------------------------------------------------------------------------- #
# 15. Year Lvl. ----
# Builds year-level panels and tests whether effects vary across annual comparisons.
# ---------------------------------------------------------------------------- #
## 15.1 Data ----
# ---------------------------------------------------------------------------- #
#' Start estimating annual differences by loading the databases
#' and aggregating them to the municipality level.
...
# 1) Open the database:
base <- data.frame() # Create an empty database that will be filled with annual data
...
for(ano in 2013:2019) { # Loop over the yearly databases
  message("Loop start for: ", ano)
  ini <- Sys.time()
  # Open each year database
  base_temp <- readRDS(file = path_paste(no_age_path, "base_nota_", ano, ".RDS")) %>% 
    select( # Select only the main columns
    ...
    filter(
      conclusao == 2, # Keep only graduating students
      priv0 == 1
      ) %>% 
  ...
  inpe <- readRDS(path_paste(inpe_mun_path, "inpe_mun_", ano, ".rds")) %>% 
    setDT()
  ...
  # Save the database aggregated at the municipality level:
  saveRDS(base_nota, file = path_paste(aggregate_path, "data_", ano, "_municipality_aggregated_controls.RDS"))
# Original output: base_nota_ag_contols{ano}.RDS
  message("saved agreggated data. Now binding to final DF.")
  
  base <- rbind(base, base_nota) %>%  # Join the databases
    setDT()
  message("Finished for ", ano)
  
  rm(base_temp, base_nota) # Remove objects that will no longer be used
  fim <- Sys.time()
...
ano_list <- c(2014:2019) # Years compared in the loop
...
ggsave(plot = p, filename = file.path(controls_figures_path, "fig_year_differences_final_v01.png"), device = "png", height = 7, width = 10)
# Original output: anos_dif_final.png
...
base <- readRDS(file = path_paste(all_observations_path, "base_nota_2019_trei.RDS")) %>%
  bind_rows(readRDS(file = path_paste(all_observations_path, "base_nota_2018_trei.RDS"))) %>%
...
# ---------------------------------------------------------------------------- #
#### 15.3.1.4 Federal Public School ----
# ---------------------------------------------------------------------------- #
...
list[[as.character(paste0(2019,"-",2018,"C|PubSF"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_psf$dn_gel[base_psf$ano == 2019], # refrigerator
    base_psf$dn_cel[base_psf$ano == 2019], # cellphone
...
list[[as.character(paste0(2019,"-",2018,"C|PubEsd"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_et$dn_gel[base_et$ano == 2019], # refrigerator
    base_et$dn_cel[base_et$ano == 2019], # cellphone
...
list[[as.character(paste0(2019,"-",2018,"C|PubFed"))]] <- rdrobust(
  ...
  covs = cbind(
    ...
    base_fed$dn_gel[base_fed$ano == 2019], # refrigerator
    base_fed$dn_cel[base_fed$ano == 2019], # cellphone
...
writeLines(latex_table, file.path(controls_output_path, "tbl_2019_2018_sample_comparison_v01.tex"))
# Original output: comp_amostras_v1.tex
